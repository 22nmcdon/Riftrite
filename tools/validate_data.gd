...
func _init() -> void:
	...
	if db.is_valid():
		print("data/ OK: %d statuses, %d essences, %d alloys, %d items, %d heroes, %d enemies, %d encounters, %d relics" % [
			db.status_ids.size(), db.essence_ids.size(), db.alloy_ids.size(), db.item_ids.size(),
			db.hero_ids.size(), db.enemy_ids.size(), db.encounter_ids.size(), db.relic_ids.size()])
		quit(0)
		return
