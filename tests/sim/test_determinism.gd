...
## a mid-fight level-up to Resonant that starts a spill, and alloys (Inferno,
## Plasma's jumping burn).
func _chaotic_fight(seed_value: int) -> FightSetup:
	...
	return K.fight(
		[K.unit("warden", 420, FRONT, [K.equip(cleave, ["ember", "ember"] as Array[String])]), K.unit("striker", 300, FRONT, [K.equip(scatter, ["umbral"] as Array[String])]), K.unit("mender", 260, BACK, [K.equip(mend, ["verdant"] as Array[String], 0, 280), K.equip(scatter, ["stone"] as Array[String])])],
