...
## A unit in the fight view: portrait, name, HP bar with its shield, status
## chips, and its items, each with an icon and a cooldown bar that flashes
## when the item fires. Damage, heals, and shields float up as numbers. It
## reads the live UnitState every frame (read-only).
...
static func make(fight_unit: UnitState, shown_name: String = "") -> UnitCard:
...
func _build_items() -> void:
	...
	for item: ItemState in unit.items:
		...
		line.add_child(Glyph.item(item.def, UiStyle.rarity_color(item.def.rarity).lightened(0.2), 14))
		...
		bar.max_value = 1.0
		bar.step = 0.0
		bar.custom_minimum_size = Vector2(114, 6)
		bar.add_theme_stylebox_override("fill", UiStyle.box(UiStyle.EMBER, UiStyle.EMBER, 0))
		bar.add_theme_stylebox_override("background", UiStyle.box(UiStyle.PANEL, UiStyle.PANEL, 0))
		inner.add_child(bar)
...
func _refresh_statuses() -> void:
	...
	for status: StatusState in unit.statuses:
		...
		chip.add_child(UiStyle.label("%s %d" % [status.def.name, status.total_stacks()], 13, color.lightened(0.3)))
...
## over `seconds`.
func float_number(text: String, color: Color, seconds: float = 0.9) -> void:
	if not is_inside_tree():
		return
	var label: Label = UiStyle.label(text, 22, color)
	label.add_theme_color_override("font_outline_color", UiStyle.BACKGROUND)
