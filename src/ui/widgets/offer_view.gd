...
static func make(session: RunSession, offer: Dictionary, action: Callable, lit: bool = false) -> Control:
	...
	match offer["type"]:
		...
		_:
			...
			if offer["type"] == "relic":
				button.tooltip_text = ItemInfo.relic_text(content, offer["relic"])
...
static func _hero_card(session: RunSession, offer: Dictionary, cost: String, action: Callable) -> Control:
	...
	box.add_child(UiStyle.label(title, 16, UiStyle.HIGHLIGHT if held != null else UiStyle.TEXT))
	box.add_child(UiStyle.label(def.hero_class.capitalize() + ("  · ranks up yours" if held != null else ""), 13, UiStyle.TEXT_DIM))
	box.add_child(UiStyle.label("HP %d  ATK %d  MGK %d  DEF %d" % [def.stats.get_stat(UnitStats.Stat.HP), def.stats.get_stat(UnitStats.Stat.ATK), def.stats.get_stat(UnitStats.Stat.MGK), def.stats.get_stat(UnitStats.Stat.DEF)], 12))
	box.add_child(UiStyle.label("Basic attack: " + def.basic_attack.name, 12, UiStyle.TEXT_DIM))
	if def.backup != null:
		box.add_child(UiStyle.label("Backup: " + def.backup.name, 12, UiStyle.TEXT_DIM))
	if not str(offer.get("specialization", "")).is_empty():
		box.add_child(UiStyle.label(content.specializations[offer["specialization"]].name, 13, UiStyle.EMBER))
